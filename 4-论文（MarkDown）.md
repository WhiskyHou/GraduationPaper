...
根据设计要求，编辑界面需要在用户点击的位置创建当前选中的地图组件，那么就会涉及到用户鼠标点击位置和游戏场景坐标位置的转换。鼠标点击事件触发时，可以通过 Unity 自带的接口获取鼠标当前的屏幕位置（ScenePosition），同时 Unity 中还提供了屏幕坐标向游戏场景世界坐标（WorldPosition）转换的接口。
```C#
...
服务端处理请求和响应的实现过程如图 5.x 所示，服务器监听采用 Nodejs 中 http 模块的 createServer 方法，监听端口为 8686。
